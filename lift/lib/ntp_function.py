...
class NTPscan:
    ...
    def monlist_scan(self,target):
        ...
        Perform a UDP Ping which will produce ICMP Port unreachable errors 
        from live hosts. 
        '''
        ...
        while (n < 3):
            ...
            # sent. The packets must be layer 3 packets (IP, ARP, etc.)
            # Send the single SYN packet `pck` to `target`, the given IP address
            # on its port 123 (which is likely to be a closed port?).
            # Quit after receving a single response.
