...
def getheaders_ssl(dest_ip, dport, cert, vbose, ctx, ssl_only, info):
    ...
    try:
        checkheaders = urllib2.urlopen(hostname, context=ctx, timeout=5)
        try:
...
def getheaders(dest_ip, dport, vbose, info):
    ...
    try:
        ...
        elif str("Radiant Device Brower").decode("utf-8") in str(title_contents) and str(server) is str("thttpd/2.25b 29dec2003"):
